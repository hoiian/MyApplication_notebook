...
public class IatDemo<command> extends Activity implements OnClickListener {
	...
	private String[] command_to = {"改成","to","for"};
	private String[] command_delete_sen_contain = {"delete the sentence have","delete the sentence include","delete the sentence contain","delete the sentence about","删掉那句有"};
	private String[] command_repeat_sen_contain = {"read the sentence about","repeat the sentence about","repeat the sentence include","读那句有"};
	//	有某字的那句删掉
	//	中间穿插一句
	//	加标点
	//	删掉UM..AH..
...
//			break;
	...
	private void printResult(RecognizerResult results) {
		...
		if (complete != true)
			return;
		for(int i=0; i<command_repeat_last_sen.length; i++ ) {
			if (result_now.contains(command_repeat_last_sen[i])) {
...
//		command_delete_sen_contain
		...
		for(int i=0; i<command_repeat_sen_contain.length; i++ ) {
			if (result_now.contains(command_repeat_sen_contain[i])) {
				Log.i("listen", "complete");
				String repeat_word = result_now.substring(result_now.indexOf(command_repeat_sen_contain[i]) + command_repeat_sen_contain[i].length(),result_now.length() - 1);
				...
				if (now_sentence > -1) {
					...
					for (int j = 0; j <= now_sentence; j++) {
						if (sentences[j].contains(repeat_word)) {

							int code = mTts.startSpeaking(sentences[j], mTtsListener);
						}else{
							int code = mTts.startSpeaking("There are no sentence contains" + repeat_word, mTtsListener);
						}
					}
